const { getKnownWords } = require("./knownWordsHelper");

// Filters OCR text into unique unknown German words
async function filterText(text, uid) {
  ...
  // Turn OCR text into individual words
  const words = text
    .split(/\s+/)
  ...
  const known = await getKnownWords(uid);
  ...
  // Remove stopwords and known words
  const filtered = words.filter(word => {
    const lower = word.toLowerCase();

    return (
      !germanStopwords.has(lower) &&
      !knownSet.has(lower)
    );
  });
